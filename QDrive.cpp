...
float QDrive::wrap(float value, const float min, const float max) {
...
auto QDrive::calibrate() -> CalibrationStatus {
...
auto QDrive::current_calibrate() -> CalibrationStatus {
    if (!enabled) return CalibrationStatus::EnvironmentError; // 如果没有使能,则不能校准
    if (started) return CalibrationStatus::Busy;              // 如果已经启动,则不能校准
...
auto QDrive::anticogging_calibrate() -> CalibrationStatus {
    if (!enabled) return CalibrationStatus::EnvironmentError;         // 如果没有使能,则不能校准
...
void QDrive::UpdateCurrent(const float iu, const float iv, const float iw) {
    /**1.保存电流值**/
    Iu = iu;
    Iv = iv;
    Iw = iw;
...
void QDrive::updateVoltage(const float voltage) {
    if (voltage == 0) return;
